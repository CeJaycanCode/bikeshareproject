...
def get_filters():
    ...
    while(True):
        ...
        if(inp == -1):
            ...
            while True:
                city = str(input('Kindly select a city from Chicago, New York City or Washington. \n')).lower()
                #print(city)
...
def user_stats(df):
    ...
    print('We are now running through Counts of user types, for example are they a susciber:\n',counts_user_types, '\n')
...
def raw_data(df):
    ...
    row_count = 0
    answer = input(
        'Would you like to view the first 5 rows? \n1-Yes\n2-No \n ').strip().lower()
    while True:
        if answer in ('yes', '1'):
            row_count += 5
